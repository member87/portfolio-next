...
export default class HomeGitCard extends Component<HomeGitProps, HomeGitState> {
  ...
  render() {
    return (
      <div
        className={`${
          this.props.invert ? "bg-background" : "bg-background-secondary"
        } p-5 flex flex-col hover:shadow-lg duration-150`}
      >
        <div className="flex flex-wrap">
          <Link href={`/view/${this.props.project}`}>
            <h4 className="flex-auto hover:underline underline-offset-8 text-xl mb-1 hover:cursor-pointer">
              {this.props.project}
            </h4>
          </Link>
          <div
            ...
            } text-paragraph text-sm rounded-full px-3 flex items-center`}
          >
            <div
              className={`w-2 h-2 mr-2 rounded-full inline-block bg-${this.state.language}`}
            ></div>
            {this.state.language}
          </div>
        </div>
        {this.state.loaded ? (
          <p className="text-paragraph flex-auto py-4">
