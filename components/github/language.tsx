    ...
    return (
      <>
        ...
        <div className="w-full h-2 bg-accent rounded-full overflow-hidden flex mb-4">
          ...
          {Object.entries(this.state.json).map(([key, value]) => {
            return (
              <div key={key} className={`bg-${key} h-2`} style={{ width: (value / this.state.count) * 100 + "%" }}>
              </div>
            )
          })}
          ...
          {Object.entries(this.state.json).map(([key, value]) => {
            return (
              <div key={key} className={`text-${key} inline-block mr-3 mb-2`}>
                <div className={`w-2 h-2 bg-${key} rounded-full inline-block mr-1`}></div>

                <span className="font-bold">{key} </span>
                <span className="text-paragraph">{((value / this.state.count) * 100).toFixed(1)}%</span>
              </div>
            )
          })}
        </div>
